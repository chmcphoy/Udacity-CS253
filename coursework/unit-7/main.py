...
class Handler(webapp2.RequestHandler):
	...
	self.can_post = self.user and self.user.name == 'lelouch'
    ...
    def notfound(self):
	self.error(404)
	self.write('<h1>404: Not Found</h1>Sorry, my pal, but that page does not exist.')
### user stuff (in utils?)
...
class Signup(Handler):
	...
	if not next_url or next_url.startswith('/login'):
        ...
        if not valid_username(self.username):
            ...
            params['error_username'] = "That name already exists."
            have_error = True
     ...
     #      have_error = True
	    ...
	    self.login(u)
	    self.redirect(next_url) # anticipate
...
class EditPage(Handler):
	...
	# When empty content is sumbmitted
